...
/* ═══════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════ */
const T = {
  bg: "#0B1426", bgSub: "#0F1A2D", bgCard: "#131F35", bgInset: "#08111F",
  border: "rgba(148,163,184,0.13)", borderL: "rgba(148,163,184,0.22)",
...
export default function App() {
  ...
  const productMode = Boolean(user && !PUB.includes(page));

  return (
    <div className={productMode ? "product-root" : ""} style={{ minHeight: "100vh", background: productMode ? "#0B1426" : T.bg, color: T.text, fontFamily: "'Inter','DM Sans',system-ui,sans-serif" }}>
      <style>{CSS}</style>
...
function Btn({ children, variant = "primary", size = "md", icon, loading: busy, onClick, disabled, style: ext, title, type = "button" }) {
  const sz = { sm: { h: 30, px: 12, fs: 11 }, md: { h: 36, px: 15, fs: 12 }, lg: { h: 44, px: 22, fs: 13 } }[size] || { h: 36, px: 15, fs: 12 };
  const va = {
    primary: { bg: T.accent, color: "#000", border: "none" },
    ghost: { bg: "rgba(255,255,255,.035)", color: T.muted, border: `1px solid ${T.borderL}` },
    outline: { bg: "rgba(56,189,248,.04)", color: T.accent, border: `1px solid rgba(56,189,248,.24)` },
  ...
  return (
    <button type={type} onClick={onClick} disabled={disabled || busy} title={title}
      style={{ height: sz.h, padding: `0 ${sz.px}px`, borderRadius: 8, border: va.border, background: va.bg, color: va.color, fontSize: sz.fs, fontWeight: 800, display: "inline-flex", alignItems: "center", gap: 6, cursor: disabled || busy ? "not-allowed" : "pointer", opacity: disabled || busy ? 0.4 : 1, flexShrink: 0, transition: "opacity .15s, background .15s, border-color .15s", ...ext }}>
...
function Nav({ page, setPage, user, onAuth, onOut, backendOk }) {
  ...
  const headerStyle = {
    ...
    background: lightNav ? "rgba(255,255,255,.97)" : "rgba(11,20,38,.96)",
    borderBottom: lightNav ? "1px solid #E2E8F0" : "1px solid rgba(148,163,184,.10)",
    ...
    <>
      <header style={headerStyle}>
        ...
        <button onClick={() => go(brandTarget)} style={{ display: "flex", alignItems: "center", gap: 9, background: "none", border: "none", cursor: "pointer", flexShrink: 0 }}>
          <div style={{ width: publicNav ? 26 : 28, height: publicNav ? 26 : 28, borderRadius: publicNav ? 5 : 7, background: "linear-gradient(135deg, #1D4ED8, #38BDF8)", display: "grid", placeItems: "center", boxShadow: publicNav ? "none" : "0 10px 26px rgba(56,189,248,.14)" }}>
            {publicNav ? <Menu size={13} color="#fff" /> : <Layers size={14} color="#fff" />}
          </div>
          ...
          <nav style={{ display: "flex", alignItems: "center", position: "absolute", left: "50%", transform: "translateX(-50%)" }}>
            {navItems.map(n => (
              <button key={n} onClick={() => go(n)} style={publicNav ? { border: "none", background: "transparent", color: linkColor(page === n), fontSize: 13.5, fontWeight: page === n ? 600 : 500, padding: "0 12px", height: 60, cursor: "pointer", position: "relative", display: "inline-flex", alignItems: "center" } : NAV_LINK(page === n)}>
                {n}
                {page === n && (
                  <span style={{ position: "absolute", bottom: 0, left: publicNav ? 12 : 16, right: publicNav ? 12 : 16, height: publicNav ? 2 : 2, background: publicNav ? "#2563EB" : T.accent, borderRadius: 2 }} />
                )}
              </button>
            ))}
          </nav>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
          {isMobile ? (
            ...
            <>
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "5px 12px 5px 6px", borderRadius: 8, background: "rgba(15,26,45,.72)", border: `1px solid ${T.borderL}` }}>
                <div style={{ width: 24, height: 24, borderRadius: 6, background: "linear-gradient(135deg, #2563EB, #38BDF8)", display: "grid", placeItems: "center", color: "#fff", fontSize: 11, fontWeight: 800 }}>{user.initials}</div>
                <span style={{ fontSize: 13, color: T.text, fontWeight: 700 }}>{user.name}</span>
                <span style={{ fontSize: 9, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".06em", padding: "2px 6px", borderRadius: 5, background: user.role === "admin" || user.role === "owner" ? "rgba(56,189,248,.12)" : "rgba(255,255,255,.05)", color: user.role === "admin" || user.role === "owner" ? T.accent : T.muted, fontFamily: "'JetBrains Mono'", border: `1px solid ${user.role === "admin" || user.role === "owner" ? "rgba(56,189,248,.22)" : "rgba(148,163,184,.10)"}` }}>{user.role || "analyst"}</span>
              </div>
              <button onClick={onOut} style={{ padding: "7px 10px", borderRadius: 8, border: `1px solid ${T.borderL}`, background: "rgba(255,255,255,.025)", color: T.muted, fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center" }}>
                <LogOut size={13} />
              </button>
            </>
          ) : (
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 49, background: "rgba(0,0,0,.7)", backdropFilter: "blur(4px)" }} onClick={() => setMenuOpen(false)}>
          <div onClick={e => e.stopPropagation()} style={{ position: "absolute", top: 56, right: 0, bottom: 0, width: 260, background: T.bgSub, borderLeft: "1px solid rgba(255,255,255,.08)", display: "flex", flexDirection: "column", padding: "16px 12px", gap: 2, overflowY: "auto" }}>
            {navItems.map(n => {
              const active = page === n;
              return (
                <button key={n} onClick={() => go(n)} style={{ border: "none", borderRadius: 8, padding: "11px 14px", textAlign: "left", fontSize: 13, fontWeight: active ? 800 : 600, background: active ? "rgba(56,189,248,.10)" : "transparent", color: active ? T.accent : T.muted, cursor: "pointer" }}>
                  {n}
                </button>
              );
            })}
...
function PublicDemoPage({ nav, auth }) {
  ...
  const steps = ["Create demo workspace", "Scan public sources", "Verify evidence", "Generate brief"];
  ...
  return (
    <main className="upgrade-demo-page">
      ...
      <section className="upgrade-demo-hero">
        ...
        <p>Pick a scenario. We'll monitor real companies, pull live web evidence, reason over business impact, and show you a decision-ready brief — in under 2 minutes.</p>
      </section>
      ...
      {phase === "running" && (
        <section className="upgrade-running">
          <div className="upgrade-running-head">
            <div style={{ width: 28, height: 28, border: `2.5px solid ${selected.color}`, borderTopColor: "transparent", borderRadius: "50%", animation: "spin .8s linear infinite", flex: "none" }} />
            <div>
              <strong>Running scenario</strong>
              <span>{selected.name || selected.hook}</span>
            </div>
            <em>~60 seconds</em>
          </div>
          <div className="upgrade-run-progress"><div style={{ width: `${Math.min(100, ((step + 1) / steps.length) * 100)}%`, background: selected.color }} /></div>
          ...
          {tab === "memory" && (
            <div className="upgrade-tab-body upgrade-memory-result">
              <div>
                ...
                <p>{brief.graph_explanation || "Every run adds to the decision memory: what changed, which evidence supported it, which action was recommended, and how entities relate."}</p>
...
/* ═══════ SHARED ═══════ */
function Eye({ children }) { return <div style={{ fontSize: 11, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".11em", color: T.accent }}>{children}</div>; }
function Lb({ children, style }) { return <div style={{ fontSize: 10, fontWeight: 800, color: T.dim, textTransform: "uppercase", letterSpacing: ".07em", ...style }}>{children}</div>; }
function MC({ l, v, c }) { return <div style={{ padding: "10px 12px", borderRadius: 8, background: "rgba(255,255,255,.025)", border: `1px solid ${T.border}` }}><div style={{ fontSize: 10, color: T.dim, textTransform: "uppercase", letterSpacing: ".06em", fontWeight: 800 }}>{l}</div><div style={{ fontSize: 17, fontWeight: 800, color: c, marginTop: 3, fontFamily: "'JetBrains Mono'" }}>{v}</div></div>; }
/* ═══════════════════════════════════════════════════════════════════════
